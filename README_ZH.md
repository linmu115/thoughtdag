...
会话主干标题、会话卡片和图列表现在跟随会话名称更新，旧图中的 ID 名称自动在读取时修正；会话请在会话页重命名，材料与未绑定草稿仍可单独命名。见[名称修复说明](docs/changes/2026-09-15-current-session-names.md)。
...
当前 DSH 插件源码版本为 **`dsh-thoughtdag` 0.4.14-rc2.13**。这是定制 RC2 接入版；版本号对应当前代码和已在本地验证的包，不表示同名 npm 包或公网下载附件已经发布。上游插件包不包含本分支的这些修改。
...
0.4.14-rc2.11 另修复[拖动卡片时的闪烁](docs/changes/2026-09-15-drag-flicker.md)，保存与后台刷新继续保留当前画布的临时尺寸信息，现有图不需迁移。

## 原生 Agent 上下文管理

安装提供 `maintenanceNativeContext` 协议 1 的配套版本后，在当前会话主干中右键来源卡片选择 **管理来源上下文**，或从连线打开 **查看固定来源与读取位置**。面板分别显示固定授权上限、当前披露窗口、原生输入实际保留材料和历史读取覆盖。

- 展开 **按用户请求选择问答范围**，按页查看来源的真实用户请求。可 **查看这一问答** 作只读预览，再把一个或多个不连续问答加入计划窗口；点击 **保存窗口** 才提交。目录和预览都不代表 AI 已读。
- **暂停来源** 阻止后续读取；勾选“暂停时同时释放”才同时清理已有材料。**恢复来源** 不自动重载全文。
- 默认使用 **释放此来源材料** 放弃当前引用的持有。其它引用仍需要的共享材料保留；“释放整份共享材料”明确影响它列出的所有持有引用。图边和稳定定位都保留。
- **用户固定保留** 阻止模型常规释放。取消用户固定后才可释放该项。界面分开显示“等待下一次原生模型请求”“已生效”和失败；保存意图不冒充已经移除输入。
- 面板按焦点和可见状态刷新模型的修改，保留尚未提交的窗口计划。超时后核对/重试使用同一操作身份。浏览其它会话的图时，先进入其所属会话才能管理输入。

本阶段只面向 DSH 原生 Agent。旧能力组合显示禁用原因，托管引擎不承诺释放。Maintenance 的 **Obsidian 系列 → 工作区 → 所属会话** 展示对应上下文使用状态；会话阅读页的 **用户请求索引** 是独立的可重建目录，不在每张图复制历史。

变更与验证见 [原生上下文面板报告](docs/changes/2026-09-15-native-context-ui.md)。
