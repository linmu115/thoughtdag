# 历史受管说明：当前安装以 dsh/README.md 为准

# DSH 会话主干图

DSH 入口仅使用 Core 和宿主的公共会话数据、引用与写入许可接口。Maintenance 通过可选 adapter 同步数据，不改变 DAG 的读取路径。仓库独立应用保留其原有能力。
...
- 宿主：`sessionExtensionData`、`sessionReferenceContext`，均为 protocolVersion 1；可选 `sessionWriteAccess`。DAG 自行检查合法性和执行增删，持久数据保存在会话扩展层。
- Maintenance adapter：可选 `sessionExtensionSync`，负责身份与格式转换、真源同步和打开会话时的恢复，不向 DAG 提供专属数据源。
...
- 固定来源和授权通过 Core 接口解析；图不复制会话正文，不调用独立模型代理，不写 IndexedDB 会话镜像。
...
# 会话图初始卡片（0.4.14-rc2.18）

从会话打开思维图时，首次建立的图包含该会话卡片，名称读取宿主会话标题。旧版尚未编辑的初始空图会在原图身份上补齐；已有内容和后续编辑不会被重置。数据仍存放在 Core 提供的会话扩展数据层，Maintenance 可选。
