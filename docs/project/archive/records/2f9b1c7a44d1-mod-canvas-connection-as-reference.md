...
id: MOD-canvas
kind: module
title: 旧版画布模块说明：连接即确认固定来源
status: superseded
summary: 2026-09-21 之前的 MOD-canvas 正文，把连接写成先确定来源与接收方、再确认已完成回复与固定版本，并称删除先调用 Maintenance 图领域 remove。
sources:
- path: ../../../src/maintenance/ManagedGraphApp.tsx
---

# 归档正文：旧版画布模块说明（原 MOD-canvas）

> 本文件保存归档时 `records/modules/managed/canvas/overview.md` 的原正文；当前说明见 `IMP-binding-20260921` 与现行的 MOD-canvas。原记录身份 MOD-canvas 继续由现行模块记录持有。
