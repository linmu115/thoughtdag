...
| [[MOD-canvas|画布编辑和对象操作]] | 会话选择、卡片/边菜单、连接确认、保存与错误入口 | src/maintenance/ManagedGraphApp.tsx |
...
| [[MOD-host-server|当前实例服务器桥接]] | 同源与输入校验，把操作转交对应 Maintenance 服务 | dsh/lib/managed-entry.js、managed-graph.js |
...
接口导航见 [[IF-integration]]；当前行为与限制见 [[IMP-current]]。
