...
本模块不发送用户正文，也不管理图的持久修订。宿主入口还会把当前会话图声明的**上游拓扑**注入提示：只说形状、不给内容，也不因此授予读取权限，约定见 [[IF-upstream-notice]]。对外调用范围见 [[IF-suite-consumer]]；自己提供给 iframe 的约定见 [[IF-host-bridge]]。
