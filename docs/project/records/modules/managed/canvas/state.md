...
- `bound` 边（上游拓扑绑定）与 `pending`、`knowledge` 一样**不携带权限**：`relationPresentation` 只在边带 `relationId` 时才按引用状态呈现。`sameEdges` 比较边身份与全部 data 字段，用来判断导入关系是否真的改变了画布，避免每次打开都抬修订。
- 打开图时会把已有 Core 授权关系补齐为边（`importRelations` + `reconcileRelationEdges`），归档图跳过。
- 已测尺寸仅放在当前画布的内存缓存，拖动、保存和后台刷新复用；不会写入任何后端存储。该设计解决卡片反复隐藏与重测引起的闪烁。
