...
5. **会话贴纸不再是一个对象**：选中回复 → 点「会话贴纸」→ 在当前工作区（由宿主 `workspaces` 投影反查，**不再让用户选工作区**）新建一个真实会话，把选段作为引用放进新会话输入框待发送，并在新会话自己的图里写一条 `bound:<被选段会话>:<新会话>` 边。没有贴纸历史列表，也没有 `sticker` 图节点。见 [[DEC-sticker-independent-20260921]]。
6. 从节点开始时先刷新权威主干；进入非当前主干的节点会转到该会话自己的主干，准备该目标的合法入向引用。
7. 右键、卡片“⋯”、Shift+F10 与 Delete/Backspace 调用相同操作。菜单可滚动、避开视口边缘，Escape/外部点击关闭。
...
已知未修：**打开贴纸对象跳转失败**这条已随功能移除（0.4.14-rc2.26 删除贴纸对象与 `stickers` 命名空间，该失败路径的代码一并消失），本页保留仅作历史定位；注释对象的 `open-object` 路径不受影响。预览与原生上下文交给 [[MOD-source-reading]]；后端接入见 [[IF-host-bridge]] 与 [[IF-suite-consumer]]。
