...
2026-09-21 用户进一步把「引用」和「绑定」拆成两件事：引用是 Core 的逻辑，绑定是 DAG 的拓扑逻辑，连线只做单纯拓扑绑定、不带锚点。修订后的边界见 [[DEC-reference-vs-binding]]。上面一句「卡片/连线的意义是上下文传导」按此修订理解为「连线只说明拓扑传导关系」，不再等于授权读取。

## 设计带来的边界

- 多来源合流由多条独立引用表达，不重写原生会话的父子版本或原始问答。
- 添加卡片与确认权限分开；源版本不可用不能悄悄改读最新。
- 连线与授权读取分开：`bound` 拓扑边不带锚点、不建引用、不注入正文，绑定本身不产生任何读取能力。
