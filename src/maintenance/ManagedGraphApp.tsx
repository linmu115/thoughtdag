...
const OBJECT_TYPES = [{ namespace: 'annotation', label: '旧注释引用', kind: 'note' as const }, { namespace: 'obsidian-links', label: '笔记引用', kind: 'note' as const }]
const errorText = (error: unknown) => error instanceof Error ? error.message : '操作未完成，请重试。'

const nodeIcons = { session: MessageSquare, material: FileText, note: FileText, placeholder: Plus }
function ManagedNode({ data, selected }: NodeProps<CanvasNode>) {
  ...
  return <div className={`mg-node mg-node-${data.kind}${selected ? ' mg-selected' : ''}`}>
    ...
    <button className="mg-node-more nodrag" data-more="true" aria-haspopup="menu" aria-label={`${data.label || '未命名卡片'}的更多操作`}><MoreHorizontal className="mg-icon" aria-hidden="true" /></button>
...
export default function ManagedGraphApp() {
  ...
  const [sessionStickers, setSessionStickers] = useState<{ id: string; capture?: Capture; workspaceId?: string } | null>(null)
  ...
  // 会话贴纸开在当前会话自己的工作区：工作区由宿主回答一次，面板不再要求用户选择。
  // 解析失败不静默降级 —— 面板会用同一条消息把原因显示出来。
  const openStickers = useCallback((id: string, capture?: Capture) => {
    void run(async () => {
      const location = await parentRequest('current-workspace', {})
      setSessionStickers({ id, capture, ...(location.workspaceId ? { workspaceId: location.workspaceId } : {}) })
    })
  }, [run])
  ...
  useEffect(() => {
    ...
    void run(async () => { const identity = await managedApi.resolveNative(currentSession.id); setCurrentIdentity(identity); await showOwnerLatest.current(identity.logicalSessionId) })
  ...
  useLayoutEffect(() => {
    ...
    const previous = menu.returnFocus ?? window.document.activeElement as HTMLElement | null
    window.document.querySelector<HTMLElement>('[role="menu"] button:not(:disabled)')?.focus()
    return () => {
      if (!previous?.isConnected) return
      let cancelled = false, frame = 0
      const deadline = performance.now() + 200
      const cancel = () => { cancelled = true; window.cancelAnimationFrame(frame); if (pendingFocusRestore.current === cancel) pendingFocusRestore.current = null }
      const restore = () => {
        if (cancelled) return
        const active = window.document.activeElement
        if (!previous.isConnected || window.document.querySelector('[role="menu"], [role="dialog"]') || (active !== window.document.body && active !== previous)) { cancel(); return }
        // React Flow briefly hides a node while measuring it. A hidden button ignores focus.
        if (getComputedStyle(previous).visibility !== 'hidden' && previous.getClientRects().length) previous.focus({ preventScroll: true })
        if (window.document.activeElement === previous || performance.now() >= deadline) cancel()
        else frame = window.requestAnimationFrame(restore)
      }
      pendingFocusRestore.current = cancel
      queueMicrotask(restore)
    }
  }, [menu])
  useLayoutEffect(() => () => pendingFocusRestore.current?.(), [])
  useLayoutEffect(() => {
    if (!menu) return
    const place = () => {
  ...
  const attachReference = async (identity: SessionIdentity, capture: Capture, operationId: string) => {
    if (dirtyRef.current) await persist()
    const result = await parentRequest('add-reference', { targetSessionId: identity.nativeSessionId, capture, operationId }) as { referenceId: string }
  ...
  const chooseSession = (item: DirectoryItem) => run(async () => {
    // 独立部署下只有一个会话身份：目录行自己的 id。`logicalSessionId` 只对更老的
    // 托管布局存在，所以缺省时回退到 id —— 与宿主图模块完全一致。
  ...
  const openMenu = (event: { preventDefault(): void; clientX: number; clientY: number; target?: EventTarget | null }, target: Partial<Menu> = {}) => {
  ...
  return <div className="mg-app" data-view-shown={shown} tabIndex={-1} onPointerDownCapture={event => { if ((event.target as Element).closest('.react-flow__edge')) event.currentTarget.focus() }} onKeyDownCapture={event => {
    ...
    <main className="mg-main"><header className="mg-toolbar"><button title="会话贴纸" aria-label="会话贴纸" disabled={busy} onClick={() => openStickers(crypto.randomUUID())}><StickyNote className="mg-icon" aria-hidden="true" /></button><div className="mg-title-group"><input aria-label="主干名称" value={title} readOnly={!!graph.ownerSessionId} title={graph.ownerSessionId ? '名称跟随所属会话，请在会话页重命名' : undefined} disabled={!editable} onChange={event => { setTitle(event.target.value); titleRef.current = event.target.value; setDirty(true); dirtyRef.current = true }} /><span className="mg-save-state" title={document ? `修订 ${document.revision}` : undefined} data-dirty={dirty}>{archived ? dirty ? '已归档 · 本地布局未保存' : '已归档 · 只读' : dirty ? '未保存' : document ? '已保存' : '尚未打开主干'}</span></div><button title="保存主干" aria-label="保存主干" disabled={!editable} className="mg-primary" onClick={() => void run(async () => { await persist(); setNotice('主干已保存。') })}><Save className="mg-icon" aria-hidden="true" /></button><button title="刷新主干" aria-label="刷新主干" disabled={!document || busy} onClick={() => void run(async () => { await refreshGraph() })}><RefreshCw className="mg-icon" aria-hidden="true" /></button></header>
    ...
    {sessionStickers && <SessionStickerPanel key={sessionStickers.id} capture={sessionStickers.capture} sourceSessionId={sessionStickers.capture?.sourceSessionId ?? currentSession?.id ?? ''} currentSessionId={currentSession?.id} workspaceId={sessionStickers.workspaceId} onClose={() => { setSessionStickers(null); requestRefresh() }} />}
