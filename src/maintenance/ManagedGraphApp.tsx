...
export default function ManagedGraphApp() {
  ...
  return <div className="mg-app" tabIndex={-1} onPointerDownCapture={event => { if ((event.target as Element).closest('.react-flow__edge')) event.currentTarget.focus() }} onKeyDownCapture={event => {
    ...
    <aside className="mg-sidebar"><header><h1><GitBranch className="mg-icon" aria-hidden="true" />会话主干图</h1><p>上下文来源 → 接收会话</p></header>
      ...
      {library && <div className="mg-canvas-list">{canvases.filter(item => !item.deleted).map(item => <button key={item.objectId} className={`mg-canvas-item${document?.objectId === item.objectId ? ' active' : ''}`} aria-current={document?.objectId === item.objectId ? 'page' : undefined} disabled={busy} onClick={() => void run(async () => { if (dirtyRef.current) await persist(); await loadDocument(await managedApi.canvas(item.objectId)) })}>{item.title}<small>修订 {item.revision}</small></button>)}{canvasCursor && <button disabled={busy} onClick={() => void run(() => listCanvases(canvasCursor))}>加载更多</button>}</div>}
      <p className="mg-sidebar-note">每个接收会话使用自己的主干。图只保存结构、固定来源和读取位置；正文按需读取。</p>
    </aside>
    <main className="mg-main"><header className="mg-toolbar"><div className="mg-title-group"><input aria-label="主干名称" value={title} disabled={!editable} onChange={event => { setTitle(event.target.value); titleRef.current = event.target.value; setDirty(true); dirtyRef.current = true }} /><span className="mg-save-state" data-dirty={dirty}>{dirty ? '未保存' : document ? `已保存 · 修订 ${document.revision}` : '尚未打开主干'}</span></div><button disabled={!editable} className="mg-primary" onClick={() => void run(async () => { await persist(); setNotice('主干已保存。') })}><Save className="mg-icon" aria-hidden="true" />保存</button><button disabled={!document || busy} onClick={() => void run(async () => { if (dirtyRef.current) await persist(); await loadDocument(await managedApi.canvas(docRef.current!.objectId)) })}><RefreshCw className="mg-icon" aria-hidden="true" />刷新主干</button></header>
