...
import { ArrowLeft, ArrowUpRight, ChevronRight, CircleAlert, Folder, Loader2, MessageSquare, Plus, RotateCcw, StickyNote, Trash2, X } from 'lucide-react';
...
type Source = Capture;
type PreparedSource = { identity: GraphSessionIdentity; preview: GraphPreviewPage; capture: Capture & { expectedSourceVersionId: string } };
...
export function SessionStickerPanel(props: Props) {
  ...
  const targetAttempt = useRef<{ key: string; target?: GraphSessionIdentity; registered?: boolean } | null>(null);
  const preparedSource = useRef<PreparedSource | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    void run(() => props.capture ? loadCreateWorkspaces() : load(undefined, false));
  }, []);
  const create = async () => {
    if (!workspace) throw new Error('请先选择新会话所在的工作区');
    if (targetAttempt.current?.registered && targetAttempt.current.target) {
      await parentRequest('open-session', { nativeSessionId: targetAttempt.current.target.nativeSessionId });
      setOpen(false);
      return;
    }
    let prepared = preparedSource.current;
    ...
    if (source && !prepared) {
      ...
      preparedSource.current = prepared;

    }
    const targetKey = 'new:' + workspace.id;
    ...
    const target = targetAttempt.current.target ?? await knowledgeRequest<GraphSessionIdentity>('create-session', { operationId: operation.current, workspaceId: workspace.id });
    ...
    if (!targetAttempt.current.registered) {
      const result = await knowledgeRequest<{ status: string }>('write', { namespace: 'stickers', objectId: 'session-' + operation.current, expectedRevision: 0, title: target.title || '新会话', body: { kind: 'session', logicalSessionId: target.logicalSessionId, ...(origin ? { source: origin } : {}) } });
      if (result.status === 'conflict') throw new Error('会话贴纸保存冲突，请刷新列表核对；再次点击将沿用本次操作身份。');
      targetAttempt.current.registered = true;
    }
  ...
  return <div className="dsh-knowledge-backdrop" onClick={event => { if (event.target === event.currentTarget && !busy) setOpen(false); }}><section ref={panel} tabIndex={-1} className="dsh-knowledge-panel" role="dialog" aria-modal="true" aria-label="会话贴纸" aria-busy={busy} onKeyDown={event => {
    ...
    {picker ? <>
      <div className="dsh-knowledge-section-title"><h3>选择新会话所在工作区</h3></div>
      {source && <blockquote className="dsh-knowledge-source">{source.selectedText.slice(0,200)}</blockquote>}
      {workspace ? <>
        <button className="dsh-knowledge-back" disabled={busy} onClick={() => void run(() => loadCreateWorkspaces())}><ArrowLeft size={14} aria-hidden="true" />重新选择工作区</button>
        <button className="dsh-knowledge-create" disabled={busy} aria-label="在所选工作区新建会话" onClick={() => void run(() => create())}><span className="dsh-knowledge-create-icon"><Plus size={20} aria-hidden="true" /></span><span><strong>在「{workspace.title}」中新建会话</strong><small>进入空白会话，引用保留在输入框中</small></span><ArrowUpRight size={16} aria-hidden="true" /></button>
      </> : <div className="dsh-knowledge-list dsh-knowledge-directory" onScroll={event => { const el = event.currentTarget; if (directoryCursor && el.scrollHeight - el.scrollTop - el.clientHeight < 60) void run(() => loadCreateWorkspaces(directoryCursor)); }}>
        ...
        {!directory.length && !busy && <p className="dsh-knowledge-muted">暂时没有可用工作区，请先新建工作区。</p>}
      ...
      <div className="dsh-knowledge-toolbar"><div className="dsh-knowledge-tabs" role="group" aria-label="贴纸视图"><button disabled={busy} aria-pressed={!deleted} onClick={() => { setDeleted(false); void run(() => load(undefined, false)); }}>全部贴纸</button><button disabled={busy} aria-pressed={deleted} onClick={() => { setDeleted(true); void run(() => load(undefined, true)); }}>已删除</button></div><button className="dsh-knowledge-primary" disabled={busy} onClick={() => { setSource(undefined); targetAttempt.current = null; operation.current = crypto.randomUUID(); setPicker(true); void run(() => loadCreateWorkspaces()); }}><Plus size={16} aria-hidden="true" />新建贴纸</button></div>
