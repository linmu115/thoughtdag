...
rmSync(tmp, { recursive: true, force: true })
await (await import('esbuild')).build({ entryPoints: [resolve(repo, 'src/maintenance/host/index.tsx')], outfile: resolve(outDir, 'host-markers.js'), bundle: true, format: 'esm', platform: 'browser', target: 'es2022', minify: true, jsx: 'automatic', define: { 'process.env.NODE_ENV': '"production"' } })
