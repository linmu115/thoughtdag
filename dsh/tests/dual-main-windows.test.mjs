...
if(process.argv.includes('--serve')){const {url}=await fixture();console.log('SYNTHETIC_PREVIEW '+url)}
 ...
 // Clearing native selection must reveal the native new-session flow without destroying either child draft.
 await left.getByRole('textbox',{name:'合成输入框'}).click();
 await page.evaluate(()=>window.fixtureSessions.clear());
 await page.locator('.dsh-main-windows').waitFor({state:'hidden'});assert.equal(await page.locator('.fixture-column').evaluate(el=>Boolean(el.inert)),false);
 await page.getByRole('textbox',{name:'合成输入框',exact:true}).fill('native new draft');
 await right.locator('body').evaluate(()=>window.parent.postMessage({channel:'dsh-main-windows-v1',paneId:'right',type:'selected',sessionId:'c'},location.origin));
 await page.waitForTimeout(50);assert.equal(await page.evaluate(()=>window.fixtureSessions.list.getSnapshot().current),undefined);
 await page.getByRole('button',{name:'合成会话 A',exact:true}).click();await page.locator('.dsh-main-windows').waitFor({state:'visible'});
 assert.equal(await left.getByRole('textbox',{name:'合成输入框'}).inputValue(),'left draft');assert.equal(await canvas.getByRole('textbox',{name:'主干名称'}).inputValue(),'合成会话 C');
