...
async function fixture() {
 ...
 const server=createServer(async(req,res)=>{
  ...
  res.setHeader('content-type','text/html');res.end(`<html><head><meta charset="utf-8"><style>body{margin:0;--dsw-alias-bg-base:#fff;--dsw-alias-label-primary:#111;--dsw-alias-border-l2:#ddd;font:14px system-ui}#root{height:100vh}.fixture-frame{display:grid;grid-template-columns:170px minmax(0,1fr) 46px;height:100%}.fixture-column{min-width:0}[data-slot]{display:contents}.wSkVaW_root{position:relative;height:100%}.wSkVaW_headerActions{display:flex;position:static}header{position:static;height:56px;display:flex;align-items:center;padding:0 12px;border-bottom:1px solid #ddd}aside{background:#f5f5f5;padding:10px;display:flex;flex-direction:column;gap:10px}textarea{box-sizing:border-box;margin:20px;width:calc(100% - 40px);height:130px}button{cursor:pointer}</style></head><body><div id="root"><div class="fixture-frame"><aside aria-label="左侧会话栏"><button data-session=a>合成会话 A</button><button data-session=b>合成会话 B</button><button data-session=c>合成会话 C</button></aside><main class="fixture-column"><div data-slot="main"><div data-slot="main.conversation"><div class="wSkVaW_root"><header class="wSkVaW_header"><span data-title>合成会话 A</span><div class="wSkVaW_headerActions"></div></header><textarea aria-label="合成输入框" placeholder="输入留在此窗口"></textarea></div></div></div></main><aside aria-label="右侧栏"><button data-right-toggle>展开</button></aside></div></div><script src="/main.js"></script><script>${boot}</script><script src="/host.js"></script></body></html>`)
 })
